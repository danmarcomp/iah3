...
function &create_field_lang_pak_contents($old_contents, $key, $value, $language, $module)
{
	...
	{ 
		// longreach - modified regexp - original one failed on strings with semicolon
		$old_contents = preg_replace("'^[^\[\n\r]+\[\'{$key}\'\].+;$'im", '', $old_contents);
   ...
   else
   {
      ...
      if($dir_exists)
      {
         ...
         if($handle)
         {
            ...
            if(isset($GLOBALS['pageInstance']))
            	$GLOBALS['pageInstance']->reload_module_language($module, $language);
         }
         else
         {
   ...
   &$file_contents)
{
	...
	foreach($dropdown_array as $key=>$value){
		if(is_string($value))
		$dropdown_array[$key] = str_replace('$', '!D!', $value);
	}
	...
	$new_contents = str_replace('!D!', '$', $new_contents);
	return $new_contents;
}
	...
	else
	{
		...
		if($new_contents == $file_contents)
		{
			...
			if (strpos($file_contents, '?>') !== false) {
				...
				$new_contents = $file_contents .=  $new_entry . "\n";
			}
		}
	}
...
function dropdown_duplicate_check($dropdown_type, &$file_contents)
{
		...
		!empty($file_contents))
	{
		...
		if(count($result[0]) > 1)
		{
			$new_entry = $result[0][count($result[0]) - 1];
