 ...
 class ListTax{
	...
	function handleAdd() {
		...
		if($current_user->is_admin) {
			...
			$allow_empty_rate = $qb_link && !empty($_POST['record']);
			if (isset($_POST['edit']) && $_POST['edit'] == 'true' && !empty($_POST['name']) && ($allow_empty_rate || !empty($_POST['rate'])) ) {
				require_once('modules/TaxRates/TaxRate.php');
				$tax = new TaxRate();
				...
				if (!$qb_link || empty($focus->id)) {
					$tax->rate = $_POST['rate'];
				}
				$tax->save();
				$this->focus = $tax;
			} elseif ($qb_link && isset($_POST['edit']) && $_POST['edit'] == 'true' && !empty($_POST['rate']) && !empty($_POST['tax_date'])
					&& !empty($_POST['rate_only']) && !empty($_POST['record'])) {
				$tax = new TaxRate();
				if ($tax->retrieve($_POST['record'])) {
					$tax->format_all_fields();
					$tax->setRateOnDate($_POST['rate'], $_POST['tax_date']);
				}
				$this->focus = $tax;
			}
		}
