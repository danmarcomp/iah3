...
function buildTableForm($rows, $mod='Opportunities'){
	...
	foreach($rows as $row){
		...
		$form .= "<td width='1%' class='$rowColor' nowrap><a href='#'  class='listViewTdLinkS1' onclick='document.dupOpps.selectedOpportunity.value=\"${row['id']}\";document.dupOpps.submit();'>[${app_strings['LBL_SELECT_BUTTON_LABEL']}]</a>&nbsp;&nbsp;</td>";
		$wasSet = false;
		foreach ($row as $key=>$value){
				if($key != 'id'){
					if(!$wasSet){
					$form .= "<td scope='row' class='$rowColor'><a target='_blank'  class='listViewTdLinkS1' href='index.php?module=Opportunities&action=DetailView&record=${row['id']}'>$value</a></td>";
					$wasSet = true;
					}else{
					$form .= "<td class='$rowColor'><a target='_blank'  class='listViewTdLinkS1' href='index.php?module=Opportunities&action=DetailView&record=${row['id']}'>$value</a></td>";
					}
				}}
...
$showaccount = $showaccount && AppConfig::setting('site.feature.require_accounts');
...
$the_form .= <<<EOQ
        ...
        <span><input name='account_name' type='text' $disabled value="" size="16" id="qc_account_name" class="sqsEnabled" autocomplete="off"></span>
<input name='account_id' type="hidden" value='' id="qc_account_id">&nbsp;<input title="{$app_strings['LBL_SELECT_BUTTON_TITLE']}" accessKey="{$app_strings['LBL_SELECT_BUTTON_KEY']}" type="button" class="button" value='{$app_strings['LBL_SELECT_BUTTON_LABEL']}' $disabled name=btn1
...
function handleSave($prefix,$redirect=true, $useRequired=false){
	...
	if (!empty($_POST['return_id']) /* longreach - added */ && $_POST['return_module'] != 'Opportunities') {
		$return_id = $_POST['return_id'];
		require_once 'modules' . DIRECTORY_SEPARATOR . $_POST['return_module'] . DIRECTORY_SEPARATOR . $beanList[$_POST['return_module']] . '.php';
		$source = new $beanList[$_POST['return_module']];
		$source->retrieve($_POST['return_id']);
		$source->opportunity_id = $id;
		$source->save();
	} else {
		$return_id = $focus->id;
	}
