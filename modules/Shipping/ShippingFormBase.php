...
class ShippingFormBase extends QuoteFormBase {
	var $log;
	var $db;
    
	function ShippingFormBase()
	{
	...
	function checkForDuplicates($prefix){
		...
		$focus = new Shipping;
		$query = '';
		$baseQuery = 'select id, name from shipping where deleted!=1 and (';
	...
	function getForm($prefix, $mod='Shipping'){
		if(!empty($mod)){
...
EOQ;
	...
	function getFormBody($prefix, $mod='Shipping', $formname=''){
		if(!empty($mod)){
			global $current_language;
			$mod_strings = return_module_language($current_language, $mod);
		} else global $mod_strings;
