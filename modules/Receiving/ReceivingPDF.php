...
class ReceivingPDF extends QuotePDF {
	var $focus_type = 'Receiving';

	function create_filename() {
		$mod_strings =& $this->mod_strings();
		$filename_pfx = $this->get_detail_title();
		$filename = $filename_pfx . '_' . $this->focus->getField('prefix');
		$filename .= $this->focus->getField('receiving_number') . '_' . $this->focus->getField('supplier_name');
	...
	function get_detail_title() {
		$s =& $this->mod_strings();
		return $s['LBL_PDF_PACKAGING_SLIP'];
	}
	...
	function get_detail_fields() {
		...
		$row =& $this->row;
		$fields = array(
			'LBL_PDF_RECEIVING_NUMBER' => $row['prefix'] . $row['receiving_number'],
	...
	function get_note_title() {
		$s =& $this->mod_strings();
		return $s['LBL_PREPARED_RECEIVING'];
	}

	function get_email_title() {
		$s =& $this->mod_strings();
		return $s['LBL_EMAILED_RECEIVING'];
	}
