...
class PurchaseOrderFormBase extends QuoteFormBase {
	var $log;
	var $db;
    
	function PurchaseOrderFormBase()
	{
	...
	function checkForDuplicates($prefix){
		...
		$query = '';
		$baseQuery = 'select id, name, amount, date_closed  from purchase_orders where deleted!=1 and (';
	...
	function getForm($prefix, $mod='PurchaseOrders'){
		if(!empty($mod)){
		...
		$the_form .= <<<EOQ
				<form name="{$prefix}EditView" onSubmit="return check_form('{$prefix}EditView')" method="POST" action="index.php">
					<input type="hidden" name="{$prefix}module" value="PurchaseOrder">
...
EOQ;
	...
	function getFormBody($prefix, $mod='PurchaseOrders', $formname=''){
		if(!empty($mod)){
			global $current_language;
			$mod_strings = return_module_language($current_language, $mod);
		} else global $mod_strings;
...
echo $quicksearch_js;
		...
		$javascript->setSugarBean(new PurchaseOrder());
...
SCRIPT;
	...
	function handleSave($prefix,$redirect=true, $useRequired=false){
		...
		if (empty($_POST['record']) && empty($_POST['dup_checked'])) {
			$duplicatePurchaseOrder = $this->checkForDuplicates($prefix);
			if(isset($duplicatePurchaseOrder)){
				$get='module=PurchaseOrders&action=ShowDuplicates';
				...
				foreach ($focus->column_fields as $field) 
				{
					...
					{
						$get .= "&PurchaseOrders$field=".urlencode($focus->$field);
					}	
				}
				...
				$i=0;
				foreach ($duplicatePurchaseOrder as $account)
				{
				...
				//echo $get;
				//die;
				//now redirect the post to modules/PurchaseOrders/ShowDuplicates.php
		...
		if($redirect){
			handleRedirect($return_id,'PurchaseOrders');
		}else{
			return $focus;	
		}
	}
