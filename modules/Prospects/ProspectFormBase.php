...
function buildTableForm($rows, $mod=''){
	...
	foreach($rows as $row){
		...
		if ($action != 'ShowDuplicates') 
		{
			$form .= "<td width='1%' bgcolor='$bgcolor' nowrap ><a  class='listViewTdLinkS1' href='#' onClick=\"document.dupProspects.selectedProspect.value='${row['id']}';document.dupProspects.submit() \">[${app_strings['LBL_SELECT_BUTTON_LABEL']}]</a>&nbsp;&nbsp;</td>\n";
		}
		$wasSet = false;
		...
		foreach ($row as $key=>$value){
				if($key != 'id'){
					...
					if(!$wasSet){
						$form .= "<td scope='row' class='$rowColor' bgcolor='$bgcolor'><a target='_blank'  class='listViewTdLinkS1' href='index.php?module=Prospects&action=DetailView&record=${row['id']}'>$value</a></td>\n";
						$wasSet = true;
					}else{
											$form .= "<td class='$rowColor' bgcolor='$bgcolor'><a target='_blank'  class='listViewTdLinkS1' href='index.php?module=Prospects&action=DetailView&record=${row['id']}'>$value</a></td>\n";
					}
