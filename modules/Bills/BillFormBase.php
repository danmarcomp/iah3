...
class BillFormBase extends QuoteFormBase {
	var $log;
	var $db;
    
	function BillFormBase()
	{
	...
	function checkForDuplicates($prefix){
		...
		$focus = new Bill();
		$query = '';
		$baseQuery = 'select id, name, amount, date_closed  from bills where deleted!=1 and (';
	...
	function getForm($prefix, $mod='Bills'){
		if(!empty($mod)){
		...
		$the_form .= <<<EOQ
				<form name="{$prefix}EditView" onSubmit="return check_form('{$prefix}EditView')" method="POST" action="index.php">
					<input type="hidden" name="{$prefix}module" value="Bill">
...
EOQ;
	...
	function getFormBody($prefix, $mod='Bills', $formname=''){
		if(!empty($mod)){
			global $current_language;
			$mod_strings = return_module_language($current_language, $mod);
		} else global $mod_strings;
...
echo $quicksearch_js;
		...
		$javascript->setSugarBean(new Bill());
...
SCRIPT;
	...
	function handleSave($prefix,$redirect=true, $useRequired=false){
		...
		if (empty($_POST['record']) && empty($_POST['dup_checked'])) {
			$duplicateBill = $this->checkForDuplicates($prefix);
			if(isset($duplicateBill)){
				$get='module=Bills&action=ShowDuplicates';
				...
				foreach ($focus->column_fields as $field) 
				{
					...
					{
						$get .= "&Bills$field=".urlencode($focus->$field);
					}	
				}
				...
				$i=0;
				foreach ($duplicateBill as $account)
				{
				...
				//echo $get;
				//die;
				//now redirect the post to modules/Bills/ShowDuplicates.php
				header("Location: index.php?$get");
				return null;
			}
		}
		$focus->related_purchase_order_id=$_REQUEST['related_purchase_order_id'];
		
		$focus->save($check_notify);
		$return_id = $focus->id;
		$GLOBALS['log']->debug("Saved record with id of ".$return_id);
		if($redirect){
			handleRedirect($return_id,'Bills');
		}else{
			return $focus;	
		}
	}
