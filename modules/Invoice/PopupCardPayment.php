...
if (!empty($_REQUEST['do_process'])) {
	...
	if (Pear::isError($result)) {
		...
		if ($success) {
			$paymentUpd = RowUpdate::blank_for_model('Payment');

			$payment = array(
				'direction' => 'incoming',
				'assigned_user_id' => AppConfig::current_user_id(),
				'amount' => unformat_number($_REQUEST['paymentAmount']),
				'currency_id' => AppConfig::setting('company.merchant_currency'),
				'account_id' => $invoice->billing_account_id,
				'payment_date' => gmdate('Y-m-d H:i:s'),
				'customer_reference' => $result->transactionId,
				'payment_type' => 'Credit Card',
			);
			$paymentUpd->set($payment);
			$paymentUpd->save();
			$paymentId = $paymentUpd->getField('id');
			...
			$itemUpd = RowUpdate::blank_for_model('invoices_payments');
			$itemUpd->set($item);
			$itemUpd->save();
			...
			add_flash_message('<a href="index.php?module=Payments&action=DetailView&record=' . $paymentId .   '">'. $mod_strings['LBL_CC_VIEW_PAYMENT'] . '</a>', 'info');
			if ($warning) {
			...
			echo '<script type="text/javascript">window.location.href="index.php?module=Invoice&action=DetailView&record=' . $invoice->id . '"; var popup = SUGAR.popups.getCurrent(); if (popup) popup.close() ; </script>';
			exit;
		}
	}
}
