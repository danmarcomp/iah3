...
class InvoiceFormBase extends QuoteFormBase {
	var $log;
	var $db;
	...
	function checkForDuplicates($prefix){
		...
		$focus = new Invoice();
		$query = '';
		$baseQuery = 'select id, name, amount, date_closed  from invoice where deleted!=1 and (';
	...
	function getForm($prefix, $mod='Invoice'){
		if(!empty($mod)){
		...
		$the_form .= <<<EOQ
				<form name="{$prefix}EditView" onSubmit="return check_form('{$prefix}EditView')" method="POST" action="index.php">
					<input type="hidden" name="{$prefix}module" value="Invoice">
...
EOQ;
	...
	function getFormBody($prefix, $mod='Invoice', $formname=''){
		if(!empty($mod)){
			global $current_language;
			$mod_strings = return_module_language($current_language, $mod);
		} else global $mod_strings;
...
echo $quicksearch_js;
		...
		$javascript->setSugarBean(new Invoice());
...
SCRIPT;
	...
	function handleSave($prefix,$redirect=true, $useRequired=false){
		...
		if (empty($_POST['record']) && empty($_POST['dup_checked'])) {
			$duplicateInvoice = $this->checkForDuplicates($prefix);
			if(isset($duplicateInvoice)){
				$get='module=Invoice&action=ShowDuplicates';
				...
				foreach ($focus->column_fields as $field) 
				{
					...
					{
						$get .= "&Invoice$field=".urlencode($focus->$field);
					}	
				}
				...
				$i=0;
				foreach ($duplicateInvoice as $account)
				{
				...
				//echo $get;
				//die;
				//now redirect the post to modules/Invoice/ShowDuplicates.php
		...
		if($redirect){
			handleRedirect($return_id,'Invoice');
		}else{
			return $focus;	
		}
	}
...
function get_copy_address_js()
{
	...
	function copyAddressLeft(form) {
		...
		form.billing_phone.value = form.shipping_phone.value;
		form.billing_email.value = form.shipping_email.value;
		return true;
	}	
	</script>
EOQ;
