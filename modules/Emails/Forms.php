...
$ampm = $timedate->AMPMMenu('',date('H:i'));
...
function nowrap_email_js($wrapped=false) {
	$wrapped = $wrapped ? '1' : '0';
	return <<<EOS
	var email_wrapped = $wrapped;
	function toggle_email_wrap() {
		var elt = document.getElementById('email_body');
		if(! elt)
			return;
		email_wrapped = ! email_wrapped;
		elt.style.whiteSpace = email_wrapped ? 'nowrap' : '';
		elt.style.display = email_wrapped ? 'block' : '';
		elt.style.width = email_wrapped ? '80em' : '';
		elt.style.overflow = email_wrapped ? 'auto' : '';
		elt.style.border = email_wrapped ? '1px inset' : '';
		elt.style.padding = email_wrapped ? '0.25em' : '';
		var link = document.getElementById('email_wrap_link');
		if(link)
			link.innerHTML = mod_string(email_wrapped ? 'LNK_WRAP_ENABLE' : 'LNK_WRAP_DISABLE');
	}
