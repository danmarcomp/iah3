...
class CreditNoteAmountDue extends FormField {
	...
	function getRequiredFields() {
		return array('apply_credit_note', 'invoice_id', 'amount_due');
	}

	function renderListCell(ListFormatter $fmt, ListResult &$result, $row_id) {
		$row = $result->getRowResult($row_id);
		if ($row->getField('apply_credit_note'))
			return '&mdash;';
		else {
			return $result->formatted_rows[$row_id]['amount_due'];
		}
    }
}
